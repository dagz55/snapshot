...
def read_snapshot_inventory(filename):
    ...
    try:
        with open(filename, 'r', newline='') as f:
            ...
            name_column = next((h for h in headers if 'name' in h.lower()), None)
            id_column = next((h for h in headers if 'id' in h.lower()), None)
            
            if not name_column or not id_column:
                raise ValueError(f"Required columns not found. Available columns: {headers}")
            
            for row in reader:
                inventory[row[name_column]] = row[id_column]
    except Exception as e:
...
async def main():
    ...
    if not snapshot_names:
        console.print(Panel("No valid snapshots found in the list. Exiting.", border_style="red"))
        return

    if not snapshot_inventory:
        console.print(Panel("No valid snapshot inventory loaded. Exiting.", border_style="red"))
        return
