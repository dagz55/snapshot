    ...
    for attempt in range(max_retries):
        try:
            ...
            if stderr:
                ...
                continue
            locks = stdout.split() if stdout else []
            logging.info(f"Lock check for {resource_group}: {'Locked' if locks else 'No locks'}")
            return bool(locks)
        except Exception as e:
    ...
    for i, name in enumerate(snapshot_names):
        try:
            ...
            if snapshot_id:
                ...
                if len(parts) < 5:
                    logging.warning(f"Invalid snapshot ID format for {name}")
                    skipped_count += 1
                    continue
                resource_group = parts[4]
                
                if resource_group not in resource_group_status:
                    is_locked = await check_resource_group_lock(resource_group)
                    resource_group_status[resource_group] = is_locked

                if resource_group_status[resource_group] and not force_delete:
                    logging.info(f"Skipping snapshot {name} due to lock in resource group {resource_group}")
                    skipped_count += 1
                    continue
