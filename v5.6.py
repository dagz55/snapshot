...
async def get_lock_info(resource_id):
    cmd = f"az lock list --resource {resource_id} -o json"
    stdout, stderr = await run_command(cmd)
    if stderr:
        logging.error(f"Error getting lock info for {resource_id}: {stderr}")
        return []
    return json.loads(stdout)

async def remove_locks(resource_id):
    locks = await get_lock_info(resource_id)
    for lock in locks:
        remove_cmd = f"az lock delete --ids {lock['id']}"
        stdout, stderr = await run_command(remove_cmd)
        if stderr:
            logging.warning(f"Failed to remove lock {lock['id']}: {stderr}")
    return locks

async def restore_locks(locks, resource_id):
    for lock in locks:
        create_cmd = f"az lock create --name '{lock['name']}' --lock-type {lock['properties']['level']} --resource-group {resource_id.split('/')[4]} --resource-name {resource_id.split('/')[-1]} --resource-type Microsoft.Compute/snapshots"
        stdout, stderr = await run_command(create_cmd)
        if stderr:
            logging.warning(f"Failed to restore lock {lock['name']}: {stderr}")

async def delete_snapshot(snapshot_id):
    try:
        # Remove locks and save their info
        locks = await remove_locks(snapshot_id)
        ...
        if stderr and "ERROR:" in stderr:
            ...
            # Restore locks if deletion failed
            await restore_locks(locks, snapshot_id)
            return False
        else:
        ...
        # Restore locks if an exception occurred
        await restore_locks(locks, snapshot_id)
        return False
